...
string execution::getFullSchedule(vector<string> icoas){
    string res;
    vector<string> lines;
...
string execution::fetchDB(vector<string> airports_name){
    string res;
    string bash_cmd(cmd);
 //Here we check if there is DB to update
    if(!(filesystem::exists(cmd) && !filesystem::is_directory(cmd)))
        return "No bash script \"flightScanner.sh\" found";
    else{
        for (const auto & entry : airports_name) {
            bash_cmd += " " + entry;
        }
    }
    if(airports_name.size() > 0)
    {
        system("rm -r flightDB");
        ...
        if (!(std::filesystem::exists(path) && std::filesystem::is_directory(path))) {
            return "Error occured data base not loaded";
        } 

        res += "DB updated succesfuly, fetched:";
        for (const auto & entry : airports_name) {
            res += " " + entry;
        }
        for (const auto& pair : airports){
        ...
        res += "Please enter at least one airport to fetch data\n";
    }
    return res;
}
...
bool execution::zipDB(){
    ...
    const string zipFilename = "./flights_pipes/DB.zip";
...
bool execution::unzipDB(){
    bool hasDir = true;
    const std::string zipFilename = "./flights_pipes/DB.zip";
